...
public class AddSocialTest extends BaseTest {
	...
	public void createMenusOnApp(HashMap<String, String> input) throws IOException, InterruptedException {
        System.out.println("*******Entring social started!********");
	    ...
	    for (int j = 0; j<websites.size(); j++) {
            ...
            //business info Twitter
	      
            businessInfoPage.clickCloseInstagramPopupButton(driver);
            if(businessInfoPage.isEmptyTwitterUserName(driver) || 
            		businessInfoPage.isEmptyInstagramUserName(driver)) {
	        	if(businessInfoPage.isEmptyTwitterUserName(driver)) {
	        		businessInfoPage.enterTwitterUserName(driver);
	            	String twitterUserNameFile = variablesAndUrlsPage.spotID
	            			+"_"
	            			+getStringLocalDateTime()
	            			+"_TWITTER_"
	            			+personalNameFromPopupOrJson;
	            	readWriteFilePage.createMenuFile(driver, twitterUserNameFile,currentTimeString);
	            	variablesAndUrlsPage.setTwitterUserName(driver,twitterUserNameFile);
	        	 }

	            //business info Instagram
	            if(businessInfoPage.isEmptyInstagramUserName(driver)) {
	            	businessInfoPage.enterInstagramUserName(driver);
	            	String instagramUserNameFile = variablesAndUrlsPage.spotID
	            			+"_"
	            			+getStringLocalDateTime()
	    	    			+"_INSTAGRAM_"
	    	    			+personalNameFromPopupOrJson;
	            	readWriteFilePage.createMenuFile(driver, instagramUserNameFile,currentTimeString);
	            	variablesAndUrlsPage.setInstagramUserName(driver,instagramUserNameFile);
	             }
        ...
        driver.close();
        System.out.println("Entering social closing ***********");
    }
